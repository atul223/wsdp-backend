const service = require('./workforceEmployer.service');
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'Workforce record created successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'Workforce record updated successfully' });
  } catch (err) {
    next(err);
  }
}
