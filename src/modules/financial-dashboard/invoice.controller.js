...
async function list(req, res, next) {
  try {
    const { data, meta } = await service.listByBudget(req.params.budgetId, req);
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'Invoice created successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'Invoice updated successfully' });
  } catch (err) {
    next(err);
  }
}
