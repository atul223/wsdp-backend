const service = require('./ehsResourceConsumption.service');
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'Resource consumption row added successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'Resource consumption row updated successfully' });
  } catch (err) {
    next(err);
  }
}
