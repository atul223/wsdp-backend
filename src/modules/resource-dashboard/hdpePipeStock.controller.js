const service = require('./hdpePipeStock.service');
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'HDPE pipe stock record created successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'HDPE pipe stock record updated successfully' });
  } catch (err) {
    next(err);
  }
}
