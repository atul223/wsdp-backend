const service = require('./ehsIncident.service');
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'EHS incident reported successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'EHS incident updated successfully' });
  } catch (err) {
    next(err);
  }
}
