...
async function list(req, res, next) {
  try {
    const { data, meta } = await service.listByResource(req.params.resourceId, req);
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'Allocation created successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'Allocation updated successfully' });
  } catch (err) {
    next(err);
  }
}
