const service = require('./equipmentDeployment.service');
...
async function create(req, res, next) {
  try {
    ...
    return created(res, { data, message: 'Equipment deployment record created successfully' });
  } catch (err) {
    next(err);
  }
}
...
async function fullUpdate(req, res, next) {
  try {
    ...
    return success(res, { data, message: 'Equipment deployment record updated successfully' });
  } catch (err) {
    next(err);
  }
}
